...
layout: post
title:  "聊一聊深度学习中的优化器"
title_en: "Deep Learning Optimizers: From SGD to Ranger"
description_en: "A walkthrough of classic and modern deep learning optimizers — SGD, Momentum, AdaGrad, RMSProp, Adam, RAdam, LookAhead, and the Ranger combo — with intuitions and comparisons."
date:   2019-11-17
categories: [论文]
tags: [Deep Learning]
---
...
1. **Local minima and saddle points:** when the gradient is zero, the step size is also zero — training stalls.

    ![](/imgs/20191117/3.jpg)

2. **Ill-conditioned loss landscapes:** if the loss changes rapidly in one direction but slowly in another, the gradient is dominated by the steep dimension, causing oscillation in that direction while making slow progress along the flat one.
...
* **First moment** (mean of gradients) — controls direction, like momentum. Controlled by `beta1` (typically 0.9).
* **Second moment** (variance of gradients) — controls step size, like RMSProp. Controlled by `beta2` (typically 0.99).
...
In August 2019, Liyuan Liu (UIUC) proposed RAdam ([paper](https://arxiv.org/abs/1908.03265)) — an optimizer that combines the strengths of Adam and SGD with automatic, data-driven warm-up. It hit the GitHub trending list quickly.
...
Proposed by Geoffrey Hinton (yes, the backprop guy) in July 2019: *LookAhead Optimizer: k Steps Forward, 1 Step Back* ([paper](https://arxiv.org/abs/1907.08610)).
...
Programmer Less Wright did exactly this, and wrote about the results in a [Medium post](https://medium.com/@lessw/new-deep-learning-optimizer-ranger-synergistic-combination-of-radam-lookahead-for-the-best-of-2dc83f79a48d). No paper — just an implementation combining RAdam and LookAhead.
...
## References
1. Stanford CS231n Lecture 3
2. Paper links are included inline throughout the post
