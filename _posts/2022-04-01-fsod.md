...
好久好久没有写 blog 了，今晚借着修评论服务的机会（虽然好像并没修好- -），贴一篇自己之前写的小样本目标检测（Few-shot Object Detection）领域的论文笔记，拯救一下我这尘封多年的博客。
...
* 模型结构：
	![](/imgs/20220401/4.png)
* 实现细节：
	* 训练阶段：meta-train meta-test（具体还没看）
...
* 实现细节：
		a. strong baseline：（VOC split1 10-shot ，能提 5.4 个点 (5/7.4)）
		...
		* 减半 head 里面用于损失计算的 proposal 数量（ROI_HEADS.BATCH_SIZE_PER_IMAGE：512->256，但是按理说应该再把 POSITIVE_FRACTION 改成0.5才对。128:384 -> 64:192））
		b. Contrastive Proposal Encoding (CPE) Loss（+2个点 (2/7.4)）
