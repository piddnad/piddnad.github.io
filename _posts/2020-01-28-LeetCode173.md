...
title_en: "LeetCode Weekly Contest 173 — Solutions"
description_en: "Solutions for Weekly Contest 173: palindromic subsequences, restaurant filtering, Dijkstra shortest path, and a dynamic programming job scheduler."
...
The start of 2020 wasn't great. For well-known reasons, I was stuck at home — and after a few days of doing nothing, boredom won. My plan: update the blog daily with problem solutions and classical music picks. Starting with the problems.

This is [LeetCode Weekly Contest 173](https://leetcode-cn.com/contest/weekly-contest-173/).

## 1332. Remove Palindromic Subsequences

Given a string containing only `'a'` and `'b'`, you can remove any palindromic subsequence in one step. Find the minimum number of steps to empty the string.

Two key definitions:
* **Palindrome**: reads the same forwards and backwards.
* **Subsequence**: a string derived by deleting some characters without changing the order. Unlike a substring, it doesn't need to be contiguous.

This looks like a string problem but is really a logic puzzle. Since the string only contains `'a'` and `'b'`, any string can be cleared in at most 2 steps — just remove all `'a'`s then all `'b'`s. The only exception is a palindrome, which can be removed in 1 step.
...
Filter a list of restaurants by given constraints, then sort by rating and ID descending. Straightforward — the only "trick" is writing a clean comparator.
...
class Solution:
    def filterRestaurants(self, restaurants: List[List[int]], veganFriendly: int, maxPrice: int, maxDistance: int) -> List[int]:
        ...
        if veganFriendly == 1:
            ...
            ans = restaurants[:]
        ans = [i for i in ans if i[3] <= maxPrice and i[4] <= maxDistance]
...
Given a weighted graph and a distance threshold, find the city with the fewest neighbors reachable within the threshold. Ties go to the city with the higher index.

This is a textbook Dijkstra problem. I'd just finished a graph theory exam a month before — yet the implementation still gave me pause. Understanding an algorithm and implementing it fluently are genuinely different skills.

**Dijkstra recap (single-source shortest path):**

Given a cost matrix, to find shortest distances from node `i` to all others:
* Initialize `shortestPath[j] = inf` for all `j ≠ i`, `shortestPath[i] = 0`. Track visited nodes.
* Each iteration: pick the unvisited node `v` with the smallest `shortestPath[v]`, mark it visited, then **relax** all edges from `v`.
...
class Solution:
    def findTheCity(self, n: int, edges: List[List[int]], distanceThreshold: int) -> int:
        ...
        for i in range(n):
            path = self.Dijkstra(G, i, n)
            temp = sum(1 for p in path if p <= distanceThreshold)
            if temp <= min_count:
                min_count = temp
                ans = i
        return ans
    ...
    def Dijkstra(self, cost, start_point, n):
        ...
        while True:
            ...
            visited[v] = 1
            for i in range(n):
                shortestPath[i] = min(shortestPath[i], shortestPath[v] + cost[v][i])
        return shortestPath
```

## 1335. Minimum Difficulty of a Job Schedule

Partition an array into `d` segments. The cost of each segment is its maximum value. Find the partition minimizing the total cost.

This is a DP problem. The key is defining the state and transition correctly.

**State:** `dp[i][j]` = minimum cost to complete jobs `0..j` in `i+1` days.

**Transition:** To compute `dp[i][j]`, split the last day's work at some index `k` (where `i ≤ k ≤ j`): the first `i` days handle jobs `0..k-1`, and day `i+1` handles jobs `k..j`.

$$
dp[i][j] = \min_{k=i}^{j}\left( dp[i-1][k-1] + \max(jobDifficulty[k], \ldots, jobDifficulty[j]) \right)
$$

**Base case:** `dp[0][k]` = max difficulty of jobs `0..k` (all done in one day).
